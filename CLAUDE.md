...
- ✅ Приветственные сообщения при /start с WebApp кнопками и медиагруппой из 3 изображений
...
- ✅ Статические файлы (изображения) в Docker контейнере
...
**Webhook URL:** `POST /webhook/telegram` (или Long Polling)

**Что отправляется:**

1. **Медиагруппа из 3 изображений** (инструкция по установке PWA):
   - `static/welcome/step1.jpg` - первое изображение с приветственным текстом
   - `static/welcome/step2.jpg` - второй шаг инструкции
   - `static/welcome/step3.jpg` - третий шаг инструкции

2. **Сообщение с WebApp кнопкой:**
   - Текст: "Нажмите на кнопку ниже, чтобы открыть приложение:"
   - Кнопка: "Открыть приложение" (WebApp button)
   - URL: `https://faberon24.vercel.app/profile.html?X-UserID={telegram_user_id}`

**Приветственный текст на первом изображении:**
```
Добро пожаловать!
...
**Реализация:**
- Медиагруппа отправляется через метод `sendMediaGroup` (Telegram Bot API)
- Используется `bot.Request()` вместо `bot.Send()`, так как API возвращает массив сообщений
- WebApp кнопка отправляется отдельным сообщением (Telegram не поддерживает inline-кнопки в MediaGroup)
- Статические изображения копируются в Docker контейнер при сборке
- Автоматическое создание пользователя в UserService при первом `/start`
...
### 🔥 5. Отправка MediaGroup с изображениями

```go
// ВАЖНО: Используем bot.Request() вместо bot.Send() для MediaGroup
// Telegram API возвращает массив сообщений, а не одно сообщение

mediaGroupConfig := tgbotapi.NewMediaGroup(chatID, mediaGroup)
resp, err := s.bot.Request(mediaGroupConfig)  // ✅ ПРАВИЛЬНО
if err != nil {
...
if !resp.Ok {
    return fmt.Errorf("%w: %s", ErrSendMediaGroup, resp.Description)
}

// ❌ НЕПРАВИЛЬНО - вызовет ошибку unmarshaling:
// _, err := s.bot.Send(mediaGroupConfig)
```

### 🔥 6. Статические файлы в Docker

```dockerfile
# В Dockerfile.notificationservice обязательно копируем static/:
...
- ✅ Отправка медиагрупп с изображениями
- ✅ Двухрежимная работа: немедленные + отложенные уведомления
- ✅ Интеграция с UserService
- ✅ Docker-контейнеризация со статическими файлами
- ✅ Prometheus метрики
- ✅ Graceful shutdown
...
3. ✅ Команда /start → пользователь создан в UserService + получено приветствие с медиагруппой (3 изображения) + WebApp кнопка
4. ✅ Перезапуск сервиса → отложенные уведомления не потеряны
5. ✅ Медиагруппа с изображениями → отправлена корректно через bot.Request()
...
### Отправка MediaGroup с изображениями

**Проблема:** Telegram API возвращает массив сообщений при отправке MediaGroup, что вызывает ошибку unmarshaling при использовании стандартного метода `bot.Send()`.

**Решение:** Использовать `bot.Request()` вместо `bot.Send()`:
...
// ❌ НЕПРАВИЛЬНО (ошибка unmarshaling):
// _, err := s.bot.Send(mediaGroupConfig)
```
...
firstPhoto := tgbotapi.NewInputMediaPhoto(tgbotapi.FilePath("./static/welcome/step1.jpg"))
firstPhoto.Caption = "Приветственный текст"
...
- Кнопки нужно отправлять отдельным сообщением
- Статические файлы должны быть скопированы в Docker контейнер
